...
const [inputNome, inputCPF, inputEmail, inputPassword, inputPasswordConfirm, inputDate, inputTel] = document.querySelectorAll(".inputsVerificar");
...
inputPasswordConfirm.addEventListener("input", msg_erro_validade_password_confirm);
...
function msg_erro_validade_password_confirm() {
  ...
  if (!passwordConfirmValidation(inputPassword.value, inputPasswordConfirm.value)){
    inputPasswordConfirm.parentElement.children[3].innerHTML = "As senhas digitadas precisam ser idênticas"
  }
  if (passwordConfirmValidation(inputPassword.value, inputPasswordConfirm.value)){
...
function msg_erro_validade_data() {
  if (!dateValidation(inputDate.value)){
    inputPassword.parentElement.children[3].innerHTML = "Você precisa ter no mínimo 18 anos para criar uma conta!"
  }
  if (dateValidation(inputDate.value)){
    inputPassword.parentElement.children[3].innerHTML = " "
  }
...
function msg_erro_validade_tel() {
  if (!telValidation(inputTel.value)){
    inputTel.parentElement.children[3].innerHTML = "Digite um número de telefone válido!"
  }
  if (telValidation(inputTel.value)){
